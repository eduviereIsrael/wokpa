import React, {ComponentType, ReactElement, FC, useEffect, useState} from 'react'
...
import { RenderedComponent } from '@/components/EnhancedComponent';
...
const Layout = <P extends object>(
    ...
    const EnhancedComponent: React.FC<P> = (props: P): ReactElement => {
      React.useEffect(() => {
        console.log(`Component ${WrappedComponent.displayName || 'Component'} has mounted.`);
      }, []);
  ...
  useEffect(() => {
    ...
    if(UserInStorage){
      ...
      return (
        <div className= {`flex h-screen w-screen bg-mainbg ${inter.className}`}>
    ...
    EnhancedComponent.displayName = `LayoutLogger(${getDisplayName(WrappedComponent)})`;
